;//block:A012
;//ƒuƒƒbƒN‚T‚O‚P‚W‚O‚©‚çjump
;//ƒuƒƒbƒN‚T‚O‚P‚X‚O‚©‚çjump
;//ƒuƒƒbƒN‚T‚O‚Q‚O‚O‚©‚çjump
;//ƒuƒƒbƒN‚T‚O‚Q‚P‚Ow’nã‚k‚`‚r‚d‚q‚Öx
;//@konya 11/12 bg“\•t

*50210_TOP
;{SceneSet ’nã‚k‚`‚r‚d‚q‚Ö}
;//---------------------------------------------------------------
;//”wŒiF‚c‚iƒo[
;//“oêl•¨:ålŒöE—I”¿E_•Eƒ}ƒ‹ƒKƒŠ[ƒ^E—øEq
;//@@@@@d‹gEƒWƒ“E÷
;//ŠÔ‘ÑF
;//EƒeƒLƒXƒg—e—ÊF1K‘OŒã
;//---------------------------------------------------------------

[sysbt_meswin]

;//bgm17Eheartbeat
[bgm storage="BGM17"]

[ChrSetEx layer=5 chbase="mizu_k4"][ChrSetXY layer=5 x=160 y=0][trans_c cross time=150]

*1314|
[fc]
[vo_miz s="mizuki0269"]
[ns]Mizuki[nse]
"Jin...!"[pcms]

*1315|
[fc]
As everyone's tension eased, Mizuki rushed over to Jin.[pcms]

[ChrSetEx layer=5 chbase="kou_e9"][ChrSetXY layer=5 x=161 y=0][trans_c cross time=150]

*1316|
[fc]
[ns]Kousuke[nse]
"...what the... Ignoring me, huh...?"[pcms]

[ChrSetEx layer=5 chbase="yuho_a3"][ChrSetXY layer=5 x=164 y=0][trans_c cross time=150]

*1317|
[fc]
[vo_yuh s="yuho1516"]
[ns]Yuuho[nse]
"Now, now, don't get angry. We've finally calmed down."[pcms]

*1318|
[fc]
Yuuho patted Kousuke's arm, which was sulking discontentedly, with a light tap.[pcms]

[ChrSetEx layer=5 chbase="kou_e2"][ChrSetXY layer=5 x=161 y=0][trans_c cross time=150]

*1319|
[fc]
[ns]Kousuke[nse]
"...Alright, I get it."[pcms]

*1320|
[fc]
Maruko-senpai called out to Kousuke, who reluctantly eased his anger, with an[r]
exasperated look.[pcms]

[ChrSetEx layer=5 chbase="maru_a1"][ChrSetXY layer=5 x=147 y=0][trans_c cross time=150]

*1321|
[fc]
[vo_mar s="maru1456"]
[ns]Margarita[nse]
"But Kousuke, what were you planning to do if it really came to a shootout?"[pcms]

[ChrSetEx layer=5 chbase="kou_e1"][ChrSetXY layer=5 x=161 y=0][trans_c cross time=150]

*1322|
[fc]
[ns]Kousuke[nse]
"What do you mean 'what'? Obviously, I'd have to do it, right?"[pcms]

[ChrSetEx layer=5 chbase="maru_a1"][ChrSetXY layer=5 x=147 y=0][trans_c cross time=150]

*1323|
[fc]
[vo_mar s="maru1457"]
[ns]Margarita[nse]
"With a gun that isn't loaded with bullets?"[pcms]

[ChrSetEx layer=5 chbase="kou_e8"][ChrSetXY layer=5 x=161 y=0][trans_c cross time=150]

*1324|
[fc]
[ns]Kousuke[nse]
"Ah..."[pcms]

*1325|
[fc]
Kousuke was at a loss for words, his mouth agape.[pcms]

*1326|
[fc]
It seems he had forgotten... Our guns don't have any bullets in them.[pcms]

*1327|
[fc]
[ns]Wataru[nse]
"Really. You had me worried."[pcms]

[ChrSetEx layer=5 chbase="kou_e5"][ChrSetXY layer=5 x=161 y=0][trans_c cross time=150]

*1328|
[fc]
[ns]Kousuke[nse]
"...Sorry."[pcms]

*1329|
[fc]
[ns]Wataru[nse]
"Well, I understand how you feel but...nnh"[pcms]

*1330|
[fc]
Just as he tried to smile, his throat choked up. Whether it was from the tension[r]
or from talking too much, his throat felt incredibly dry.[pcms]

*1331|
[fc]
[ns]Wataru[nse]
"Ah, seriously, my throat is parched."[pcms]

[ChrSetEx layer=5 chbase="ren_a8"][ChrSetXY layer=5 x=185 y=0][trans_c cross time=150]

*1332|
[fc]
[vo_ren s="ren1353"]
[ns]Ren[nse]
"I'll go get some water."[pcms]

[ChrSetEx layer=5 chbase="yuho_a4"][ChrSetXY layer=5 x=164 y=0][trans_c cross time=150]

*1333|
[fc]
[vo_yuh s="yuho1517"]
[ns]Yuuho[nse]
"Ah, I'll go too. It'd be nice if there's juice or something."[pcms]

*1334|
[fc]
Ren and Yuuho headed towards the kitchen, laughing together cheerfully.[pcms]

;//YƒtƒF[ƒhƒAƒEƒg
[black_toplayer][trans_c cross time=1000][hide_chara_int]

;//¨ƒuƒƒbƒN‚Ö
[jump storage="50220.ks" target=*50220_TOP]

