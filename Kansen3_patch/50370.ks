;//block:A003
;//ブロック５０３７０『ＬＡＳＥＲ安全確保作戦』
;//@konya 11/12 bg貼付

*50370_TOP
;{SceneSet ＬＡＳＥＲ安全確保作戦}
;//---------------------------------------------------------------
;//背景：ＬＡＳＥＲバックヤード通路　警備ブース　ホールに出てすぐの宇田川通り出入り口
;//登場人物:主人公・悠帆・浩助・マルガリータ・漣
;//　　　　重吉・ジン・瑞樹
;//時間帯：
;//・テキスト容量：
;//---------------------------------------------------------------

;//bgmは継続(bgm004)

;//〆背景：ＬＡＳＥＲバックヤード通路
;//@konya LASER内部
[bg storage="BG08e"][trans_c cross time=500]
;[eval exp="f.l_map = 18"]

[sysbt_meswin]

*1778|
[fc]
[ns]Wataru[nse]
"...let's begin."[pcms]

*1779|
[fc]
As I hang up the phone from Yuuho, I relay the message to[r]
Maruko-senpai and Kousuke.[pcms]

*1780|
[fc]
The three of us were standing by near the security booth on[r]
the first floor.[pcms]

*1781|
[fc]
[vo_yuh s="yuho1551"]
[ns]Yuuho[nse]
"Okay, all infected people, gather over here!"[pcms]
;//＠扉越し。遠くで叫んでいる

*1782|
[fc]
[vo_miz s="mizuki0295"]
[ns]Mizuki[nse]
"Hurry up or you'll miss out on the meat!"[pcms]
;//＠扉越し。遠くで叫んでいる

*1783|
[fc]
From beyond the fire door, I could hear Yuuho and the others[r]
calling out, the sound of a violin, and the groans of moving[r]
infected people.[pcms]

[ChrSetEx layer=5 chbase="maru_a1"][ChrSetXY layer=5 x=147 y=0][trans_c cross time=150]

*1784|
[fc]
[vo_mar s="maru1502"]
[ns]Margarita[nse]
"...then, as per our discussion. Is that clear?"[pcms]

[ChrSetEx layer=5 chbase="kou_e11"][ChrSetXY layer=5 x=161 y=0][trans_c cross time=150]

*1785|
[fc]
[ns]Kousuke[nse]
"Yeah!"[pcms]

*1786|
[fc]
[ns]Wataru[nse]
"Okay!"[pcms]

*1787|
[fc]
Holding several dark curtains brought from the DJ bar stage[r]
and restaurant, Kousuke and I respond.[pcms]

[ChrSetEx layer=5 chbase="maru_a7"][ChrSetXY layer=5 x=147 y=0][trans_c cross time=150]

*1788|
[fc]
[vo_mar s="maru1503"]
[ns]Margarita[nse]
"GO!"[pcms]

;//キャラ消し
[chara_int][trans_c cross time=150]

*1789|
[fc]
At the signal of her voice, we all start moving together.[pcms]

*1790|
[fc]
[ns]Wataru[nse]
"...good, no one's here!"[pcms]

*1791|
[fc]
I open the door to the security booth, check inside, and[r]
roll into the booth covered head to toe in a dark curtain.[pcms]

*1792|
[fc]
[ns]Wataru[nse]
"Ready... go!"[pcms]

;//se063・シートを広げる
[se buf=0 storage="se063"]

*1793|
[fc]
Then I spread the curtain as wide as possible, covering the[r]
glass walls of the security booth that are completely[r]
visible from outside.[pcms]

*1794|
[fc]
[ns]Wataru[nse]
"Please go ahead, senpai!"[pcms]

[ChrSetEx layer=5 chbase="maru_a7"][ChrSetXY layer=5 x=147 y=0][trans_c cross time=150]

*1795|
[fc]
[vo_mar s="maru1504"]
[ns]Margarita[nse]
"Roger that!"[pcms]

*1796|
[fc]
Maruko-senpai jumps in with an air nail gun in hand, looking[r]
like a ray gun from a sci-fi movie.[pcms]

*1797|
[fc]
I'm stretching to support the curtain when senpai's body[r]
bounces against me with force.[pcms]

;//キャラ消し
[chara_int][trans_c cross time=150]

*1798|
[fc]
[ns]Wataru[nse]
"...uh..."[pcms]

*1799|
[fc]
Since it's a single-person booth, senpai's body is pressed[r]
tightly against mine.[pcms]

*1800|
[fc]
The feeling of her breasts... soft skin... All the wonderful[r]
elements of a young girl are transmitted to me, and I can't[r]
help but feel entranced.[pcms]

[ChrSetEx layer=5 chbase="maru_a6"][ChrSetXY layer=5 x=147 y=0][trans_c cross time=150]

*1801|
[fc]
[vo_mar s="maru1505"]
[ns]Margarita[nse]
"...okay, this is good!"[pcms]

*1802|
[fc]
Senpai quickly operates the nail gun, fixing the curtain to[r]
the ceiling and then looks up at me and nods.[pcms]

[ChrSetEx layer=5 chbase="maru_a8"][ChrSetXY layer=5 x=147 y=0][trans_c cross time=150]

*1803|
[fc]
[vo_mar s="maru1506"]
[ns]Margarita[nse]
"Wataru, let's move on to the next one."[pcms]

*1804|
[fc]
This position... it's almost like we're hugging each other.[pcms]

*1805|
[fc]
A sweet scent rises from senpai's skin...[pcms]

[ChrSetEx layer=5 chbase="maru_a16"][ChrSetXY layer=5 x=147 y=0][trans_c cross time=150]

*1806|
[fc]
[vo_mar s="maru1507"]
[ns]Margarita[nse]
"...Wataru...?"[pcms]
;//＠ぼーっとした主人公に気付いて

*1807|
[fc]
The tips of her silky hair tickle my neck...[pcms]

[ChrSetEx layer=5 chbase="maru_a4"][ChrSetXY layer=5 x=147 y=0][trans_c cross time=150]

*1808|
[fc]
[vo_mar s="maru1508"]
[ns]Margarita[nse]
"Are you okay?"[pcms]
;//＠心配

*1809|
[fc]
Senpai's face comes closer... If she gets any closer, I[r]
won't be able to handle it...[pcms]

*1810|
[fc]
[ns]Wataru[nse]
"...eh?"[pcms]

*1811|
[fc]
When I come to my senses, Maruko-senpai is looking into my[r]
eyes with concern.[pcms]

*1812|
[fc]
[ns]Wataru[nse]
"Ah... ah, yes, I'm fine, nothing's wrong!"[pcms]

[ChrSetEx layer=5 chbase="maru_a2"][ChrSetXY layer=5 x=147 y=0][trans_c cross time=150]

*1813|
[fc]
[vo_mar s="maru1509"]
[ns]Margarita[nse]
"If that's the case then good... Being too tense can slow[r]
you down when it matters. Try to relax as much as possible."[pcms]

*1814|
[fc]
[ns]Wataru[nse]
"Yes..."[pcms]

*1815|
[fc]
It seems she thought I was just tense and stiff.[pcms]

*1816|
[fc]
In this case, it might be best to leave the misunderstanding[r]
as it is...[pcms]

[ChrSetEx layer=5 chbase="maru_a4"][ChrSetXY layer=5 x=147 y=0][trans_c cross time=150]

*1817|
[fc]
[vo_mar s="maru1510"]
[ns]Margarita[nse]
"And... ah, this is it."[pcms]

*1818|
[fc]
Searching through a drawer, senpai finds a walkie-talkie[r]
used for communication by security guards and picks it up.[pcms]

[ChrSetEx layer=5 chbase="maru_a2"][ChrSetXY layer=5 x=147 y=0][trans_c cross time=150]

*1819|
[fc]
[vo_mar s="maru1511"]
[ns]Margarita[nse]
"Wataru. This one's for you."[pcms]

*1820|
[fc]
[ns]Wataru[nse]
"Ah, yes."[pcms]

*1821|
[fc]
We each put on a walkie-talkie and switch them on.[pcms]

[ChrSetEx layer=5 chbase="maru_a8"][ChrSetXY layer=5 x=147 y=0][trans_c cross time=150]

*1822|
[fc]
[vo_mar s="maru1512"]
[ns]Margarita[nse]
"...can you hear me?"[pcms]
;//＠インカムのボイスと、肉声が同時に聞こえます

*1823|
[fc]
[ns]Wataru[nse]
"Yes. Reception is good."[pcms]

[ChrSetEx layer=5 chbase="maru_a2"][ChrSetXY layer=5 x=147 y=0][trans_c cross time=150]

*1824|
[fc]
[vo_mar s="maru1513"]
[ns]Margarita[nse]
"I'm counting on you for the rest."[pcms]

;//キャラ消し
[chara_int][trans_c cross time=150]

*1825|
[fc]
After confirming communication status, senpai pats my[r]
shoulder once and returns to the hallway.[pcms]

*1826|
[fc]
[ns]Wataru[nse]
"...yes sir!"[pcms]

*1827|
[fc]
I reply belatedly and take a deep breath.[pcms]

*1828|
[fc]
Calm down, me. ...Ah, but taking a deep breath just spreads[r]
senpai's nice scent from my nose to my brain...[pcms]

*1829|
[fc]
[ns]Wataru[nse]
"...This is no time for that!"[pcms]

*1830|
[fc]
While slapping my own face to regain composure, I focus on[r]
the monitor screens inside the booth.[pcms]

*1831|
[fc]
Not only the entrance hall on the first floor, but also the[r]
emergency exit at the back and the delivery entrance camera[r]
feeds are displayed on the split screens.[pcms]

*1832|
[fc]
[ns]Wataru[nse]
"...They're moving..."[pcms]

*1833|
[fc]
The infected inside the hall were slowly starting to head[r]
out towards Udagawa Street.[pcms]

*1834|
[fc]
The number of them was smaller than I had imagined, and[r]
after a short wait, the hall was empty.[pcms]

*1835|
[fc]
[ns]Wataru[nse]
"...Senpai, Kousuke. It's OK!"[pcms]

*1836|
[fc]
I speak into the walkie-talkie, timing it right.[pcms]

*1837|
[fc]
The monitor shows the two of them raising their hands in[r]
acknowledgment before opening the fire doors and heading out[r]
into the hall.[pcms]

*1838|
[fc]
[ns]Wataru[nse]
"...It's safe, right...?"[pcms]

*1839|
[fc]
The two who rushed into the hall quickly closed the two[r]
entrances on the Udagawa Street side and started hanging[r]
blackout curtains in front of the doors.[pcms]

*1840|
[fc]
Some of the infected noticed them and began banging on the[r]
glass doors trying to get back into the building.[pcms]

*1841|
[fc]
[ns]Kousuke[nse]
"It's reinforced glass, you idiots!! Too bad you can't get[r]
in!"[pcms]

*1842|
[fc]
Kousuke's voice can be heard through the walkie-talkie.[pcms]

*1843|
[fc]
Additionally, Kousuke's figure is clearly displayed on the[r]
monitor, trying to provoke them by flipping them off.[pcms]

*1844|
[fc]
[ns]Wataru[nse]
"...Haha. What are you doing, Kousuke."[pcms]

*1845|
[fc]
[vo_mar s="maru1514"]
[ns]Margarita[nse]
"What's next, Wataru?"[pcms]
;//＠インカム音声

*1846|
[fc]
...This is no time to laugh.[pcms]

*1847|
[fc]
I quickly scan the monitors again.[pcms]

*1848|
[fc]
[ns]Wataru[nse]
"Elevator entrance side, Inokashira street side, both are[r]
clear. No infected in sight!"[pcms]

;//[ChrSetEx layer=5 chbase="maru_a7"][ChrSetXY layer=5 x=147 y=0][trans_c cross time=150]

*1849|
[fc]
[vo_mar s="maru1515"]
[ns]Margarita[nse]
"Alright! Let's go!"[pcms]

;//[ChrSetEx layer=5 chbase="kou_e1"][ChrSetXY layer=5 x=161 y=0][trans_c cross time=150]

*1850|
[fc]
[ns]Kousuke[nse]
"Here we go!"[pcms]

;//キャラ消し
[chara_int][trans_c cross time=150]

*1851|
[fc]
There are two more entrances to secure in the entrance hall.[pcms]

*1852|
[fc]
At my signal for safety confirmation, senpai and Kousuke[r]
dash through the hall.[pcms]

*1853|
[fc]
[ns]Wataru[nse]
"Alright, that should do it..."[pcms]

*1854|
[fc]
I also leave the security booth and collect the remaining[r]
blackout curtains in the hallway before heading to the hall.[pcms]

[bg storage="BG08f"][trans_c cross time=500]
;[eval exp="f.l_map = 18"]

*1855|
[fc]
By the time I joined them, they had already sealed all the[r]
entrances and hung up the blackout curtains.[pcms]

[ChrSetEx layer=5 chbase="maru_a3"][ChrSetXY layer=5 x=147 y=0][trans_c cross time=150]

*1856|
[fc]
[vo_mar s="maru1516"]
[ns]Margarita[nse]
"...Seal complete."[pcms]

[ChrSetEx layer=5 chbase="kou_e1"][ChrSetXY layer=5 x=161 y=0][trans_c cross time=150]

*1857|
[fc]
[ns]Kousuke[nse]
"Got a bit dark though."[pcms]

*1858|
[fc]
[ns]Wataru[nse]
"Then, I'll let them know on the other side."[pcms]

;//se022・携帯の操作音
[se buf=0 storage="se022"]

*1859|
[fc]
I take out my cell phone and press the speed dial button.[pcms]

*1860|
[fc]
[ns]Wataru[nse]
"...Hello, Yuuho? We've finished up safely over here!"[pcms]

*1861|
[fc]
[vo_yuh s="yuho1552"]
[ns]Yuuho[nse]
"Ahhhh, Wataru~~~uh, I've been waiting~~~o..."[pcms]
;//＠電話越し。喉がかれて、声が枯れている

*1862|
[fc]
From the other end of the phone comes Yuuho's voice, hoarse[r]
as if she's pitifully strained it, speaking in a pathetic[r]
tone.[pcms]

*1863|
[fc]
[vo_yuh s="yuho1553"]
[ns]Yuuho[nse]
"I'm so thirsty, my voice is all gravelly~~~"[pcms]
;//＠電話越し。喉がかれて、声が枯れている

*1864|
[fc]
[ns]Wataru[nse]
"Hahaha, good work. Make sure you come back safely."[pcms]

*1865|
[fc]
[vo_yuh s="yuho1554"]
[ns]Yuuho[nse]
"You too, good work~~~. Alright, we're pulling out~~~"[pcms]

*1866|
[fc]
I hang up the phone and look around inside the hall.[pcms]

[ChrSetEx layer=5 chbase="kou_e3"][ChrSetXY layer=5 x=161 y=0][trans_c cross time=150]

*1867|
[fc]
[ns]Kousuke[nse]
"But man, that was a piece of cake!"[pcms]

[ChrSetEx layer=5 chbase="maru_a3"][ChrSetXY layer=5 x=147 y=0][trans_c cross time=150]

*1868|
[fc]
[vo_mar s="maru1517"]
[ns]Margarita[nse]
"You guys moved quickly. Efficiency was key to victory."[pcms]

*1869|
[fc]
Maruko-senpai smiled contentedly at that moment.[pcms]

;//キャラ消し
[chara_int][trans_c cross time=150]

*1870|
[fc]
[ns]Infected Man A[nse]
"...Aaaaauuuuhhh..."[pcms]

*1871|
[fc]
[vo_mob s="kan2_0001"]
[ns]Infected Woman B[nse]
"...It smells, it smells..."[pcms]

*1872|
[fc]
I hurriedly turn towards the direction of the groaning.[pcms]

*1873|
[fc]
And then I see an infected person who seems to have been[r]
left behind in Starbucks staggering out.[pcms]

*1874|
[fc]
[ns]Infected Man A[nse]
"Meat... I want meat...!"[pcms]

*1875|
[fc]
[vo_mob s="kan2_0002"]
[ns]Infected Woman B[nse]
"There's a woman too! Tasty looking, young, flesh!!"[pcms]

[ChrSetEx layer=5 chbase="kou_e10"][ChrSetXY layer=5 x=161 y=0][trans_c cross time=150]

*1876|
[fc]
[ns]Kousuke[nse]
"Don't scare me like that!"[pcms]

*1877|
[fc]
Kousuke tries to force a smile on his tense face as he[r]
glares at the infected.[pcms]

[ChrSetEx layer=5 chbase="kou_e1"][ChrSetXY layer=5 x=161 y=0][trans_c cross time=150]

*1878|
[fc]
[ns]Kousuke[nse]
"But with this many people, they're no match for us! Don't[r]
underestimate us!"[pcms]

[ChrSetEx layer=5 chbase="maru_a9"][ChrSetXY layer=5 x=147 y=0][trans_c cross time=150]

*1879|
[fc]
[vo_mar s="maru1518"]
[ns]Margarita[nse]
"Stop it!"[pcms]

*1880|
[fc]
As Kousuke was about to draw his gun, the senior stopped[r]
him.[pcms]

[ChrSetEx layer=5 chbase="kou_e9"][ChrSetXY layer=5 x=161 y=0][trans_c cross time=150]

*1881|
[fc]
[ns]Kousuke[nse]
"Wh-why..."[pcms]

[ChrSetEx layer=5 chbase="maru_a2"][ChrSetXY layer=5 x=147 y=0][trans_c cross time=150]

*1882|
[fc]
[vo_mar s="maru1519"]
[ns]Margarita[nse]
"There's a risk of stray bullets. Beyond the curtain is a[r]
glass door. If we break it, the blockade will be for[r]
nothing!"[pcms]

[ChrSetEx layer=5 chbase="kou_e4"][ChrSetXY layer=5 x=161 y=0][trans_c cross time=150]

*1883|
[fc]
[ns]Kousuke[nse]
"I-I see..."[pcms]

*1884|
[fc]
Indeed, even reinforced glass would break if hit by a[r]
bullet.[pcms]

*1885|
[fc]
But without weapons, how are we supposed to fight them...?[pcms]

*1886|
[fc]
[ns]Wataru[nse]
"Ah... I see."[pcms]

*1887|
[fc]
I realize that I'm holding a nail gun in my hand.[pcms]

;//---------------------------------------------------------------
;//●選択肢Ａ１
;//１１:マル子先輩、これなら？→ブロック５０３８０
;//１２:僕がやる！→ブロック５０３９０


;	[link storage="50380.ks" target=*50380_TOP]マル子先輩、これなら？[endlink]
;       (link storage="50390.ks" target=*50390_TOP)I'll do[r]
it!(endlink)[pcms]

*SEL45|マル子先輩、これなら？／僕がやる！
[fc]
[pcms_sel]

[eval exp="f.seltext02 = 'Senior Maruko, how about this?'"]
[eval exp="f.seltext04 = 'I\'ll do it!'"]

[if exp="tf.sys_sub == 0 || tf.選択肢ログ表示してね == 1"]
	;選択肢内容をバックログに表記。改行コード必須。
	[sel_hisout txt="&f.seltext02"][hr]
	[sel_hisout txt="&f.seltext04"][hr]
[endif]
[hr]

[履歴出力復帰]

;選択肢ベース
[selbase]
;文字の左マージン
;[eval exp="sf.seltext2_x = (800-(6*25))/2"]
;[eval exp="sf.seltext4_x = (800-(6*25))/2"]

[sel02 target=*SEL45_1]
[sel04 target=*SEL45_2]
[s]

;選択肢後の処理しときたいからここに飛ばしてから実際のjump先へ
;-------------------------------------------------------------------------------
*SEL45_1|&f.seltext02
[sel_hisout txt="&f.seltext02"][hr][fc][selbt_clear]
[jump storage="50380.ks" target=*50380_TOP]
;-------------------------------------------------------------------------------
*SEL45_2|&f.seltext04
[sel_hisout txt="&f.seltext04"][hr][fc][selbt_clear]
[jump storage="50390.ks" target=*50390_TOP]
;//---------------------------------------------------------------

