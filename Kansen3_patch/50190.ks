;//blockA010
;//ƒuƒƒbƒN‚T‚O‚P‚V‚O‚©‚ç‘I‘ğˆ‚Åjump
;//ƒuƒƒbƒN‚T‚O‚P‚X‚Ow’nã‚k‚`‚r‚d‚q‚Öx
;//@konya 11/12 bg“\•t

*50190_TOP
;{SceneSet ’nã‚k‚`‚r‚d‚q‚Ö}
;//---------------------------------------------------------------
;//”wŒiF‚c‚iƒo[
;//“oêl•¨:ålŒöE—I”¿E_•Eƒ}ƒ‹ƒKƒŠ[ƒ^E—øEq
;//@@@@@@@@d‹gEƒWƒ“E÷
;//ŠÔ‘ÑF
;//EƒeƒLƒXƒg—e—ÊF1K‘OŒã
;//---------------------------------------------------------------

;[sysbt_meswin]

*1237|
[fc]
[ns]Wataru[nse]
"Calm down... calm down, me..."[pcms]

*1238|
[fc]
If I don't do something, we're heading straight into a bloodbath of screams and[r]
chaos.[pcms]

*1239|
[fc]
I need to stay calm and persuade them... to peacefully resolve this situation...[pcms]

*1240|
[fc]
[ns]Wataru[nse]
"Um... excuse me, may I have a moment?"[pcms]

*1241|
[fc]
Breaking through the tense atmosphere, I hesitantly start to speak.[pcms]

[ChrSetEx layer=5 chbase="jin_c4"][ChrSetXY layer=5 x=144 y=0][trans_c cross time=150]

*1242|
[fc]
[ns]Jin[nse]
"Hmm?"[pcms]

*1243|
[fc]
I was glared at by Jin...! It was so scary that the hairs on the back of my neck[r]
stood up.[pcms]

*1244|
[fc]
[ns]Wataru[nse]
"Ah, um... thank you for saving Mizuki."[pcms]

*1245|
[fc]
But I can't back down here... I spoke as calmly as I could.[pcms]

*1246|
[fc]
[ns]Wataru[nse]
"However... that doesn't mean I can completely trust you guys who are waving[r]
guns around..."[pcms]

*1247|
[fc]
[ns]Wataru[nse]
"So please answer our questions. If you do, Kousuke will be satisfied, we can[r]
end this situation, and all relax together..."[pcms]

*1248|
[fc]
Seeing that no one was responding, I decided to continue trying to persuade[r]
them.[pcms]

*1249|
[fc]
[ns]Wataru[nse]
"I'm thirsty, and I'd like to turn up the cooler a bit more. And if we go to a[r]
manga cafe, we can even take showers... Ah, um, just washing our faces would be[r]
fine too..."[pcms]

[ChrSetEx layer=5 chbase="jin_c3"][ChrSetXY layer=5 x=144 y=0][trans_c cross time=150]

*1250|
[fc]
[ns]Jin[nse]
"...You..."[pcms]

*1251|
[fc]
Jin started to say something but stopped, his face angry.[pcms]

[ChrSetEx layer=5 chbase="jin_c4"][ChrSetXY layer=5 x=144 y=0][trans_c cross time=150]

*1252|
[fc]
[ns]Jin[nse]
"...Fine, I get it. Shige, put that away."[pcms]

*1253|
[fc]
A heavy sigh was heard, and the gun was tucked away.[pcms]

[ChrSetEx layer=5 chbase="shige_b2"][ChrSetXY layer=5 x=161 y=0][trans_c cross time=150]

*1254|
[fc]
[ns]Shigeyoshi[nse]
"But, Aniki...!"[pcms]

[ChrSetEx layer=5 chbase="jin_c3"][ChrSetXY layer=5 x=144 y=0][trans_c cross time=150]

*1255|
[fc]
[ns]Jin[nse]
"Put it away!"[pcms]

[ChrSetEx layer=5 chbase="shige_b2"][ChrSetXY layer=5 x=161 y=0][trans_c cross time=150]

*1256|
[fc]
[ns]Shigeyoshi[nse]
"...Okay, understood."[pcms]

*1257|
[fc]
Reluctantly, Shigeyoshi obeyed Jin's words.[pcms]

[ChrSetEx layer=5 chbase="maru_a2"][ChrSetXY layer=5 x=147 y=0][trans_c cross time=150]

*1258|
[fc]
[vo_mar s="maru1454"]
[ns]Margarita[nse]
"..."[pcms]
;//—‚Ü‚¾‹Ù’£‚ğ‰ğ‚¢‚Ä‚Í‚¢‚È‚¢

*1259|
[fc]
Without taking her eyes off the scene, Maruko-senpai also lowered her gun in[r]
sync with their movements.[pcms]

[ChrSetEx layer=5 chbase="jin_c4"][ChrSetXY layer=5 x=144 y=0][trans_c cross time=150]

*1260|
[fc]
[ns]Jin[nse]
"..."[pcms]

*1261|
[fc]
Before I knew it, Jin was staring intently at me.[pcms]

*1262|
[fc]
[ns]Wataru[nse]
"Eh... what is it...?"[pcms]

[ChrSetEx layer=5 chbase="jin_c2"][ChrSetXY layer=5 x=144 y=0][trans_c cross time=150]

*1263|
[fc]
[ns]Jin[nse]
"...I've lost my edge."[pcms]

*1264|
[fc]
[ns]Wataru[nse]
"Huh?"[pcms]

*1265|
[fc]
The expression on Jin's face... what is this...? Could it be... he's[r]
dumbfounded...?[pcms]

[ChrSetEx layer=5 chbase="jin_c4"][ChrSetXY layer=5 x=144 y=0][trans_c cross time=150]

*1266|
[fc]
[ns]Jin[nse]
"To ask for a 'stronger cooler' in this situation...? You've got some unusual[r]
nerves..."[pcms]

*1267|
[fc]
[ns]Wataru[nse]
"I... I'm sorry."[pcms]

*1268|
[fc]
Now that he mentions it... maybe that was foolish of me...?[pcms]

*1269|
[fc]
Feeling my cheeks turn red, I mumbled to myself.[pcms]

;//¨ƒuƒƒbƒN‚T‚O‚Q‚P‚O‚Ö

;//bgm’â~
[fadeoutbgm time=500]

[jump storage="50210.ks" target=*50210_TOP]

